...
if __name__ == '__main__':
    min_amt = input("What is the minimum amount of Session you are looking to stake?\n>> ")
    max_amt = input("What is the maximum amount of Session you can stake?\n>> ")
    min_amt = int(min_amt)
    max_amt = int(max_amt)
    ...
    while True:
        try:
            ...
            if url is None:
                status_logger.logger.error("Unable to connect to an Session remote node. Please try again later.\n")
                sys.exit()
            else:
